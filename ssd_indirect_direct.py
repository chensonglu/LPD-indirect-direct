...
from layers import *
from data import indirect, carplate_branch, change_cfg_for_ssd512
import os
import numpy as np
...
class SSD_indirect_direct(nn.Module):
    ...
    def __init__(self, phase, size, size_2, base, extras, head, base_2, head_2, carplate_head, num_classes, expand_num):
        super(SSD_indirect_direct, self).__init__()
        self.phase = phase
    ...
    def forward(self, x, targets):
        ...
        if self.phase == 'test':
            ...
            output_indirect = torch.zeros(1, 3, 200, 13)
            output_direct = torch.zeros(1, 2, 200, 13)

            output[0, 1, :, :5] = rpn_rois[0, 1, :, :5]
            output_indirect[0, 1, :, :5] = rpn_rois[0, 1, :, :5]
            ...
            output_indirect[0, 2, :num_car, :] = output_2_pos
            ...
            output_direct[0, 1, :, :] = output_carplate[0, 1, :, :]

            # indirect+direct
            conf_thresh = 0.01
            nms_thresh = 0.45
            top_k = 200
            output_carplate_indirect_direct = torch.cat((output_2_pos, output_carplate[0, 1, :, :]), 0)
            output_carplate_indirect_direct = output_carplate_indirect_direct.detach()
            conf_scores = output_carplate_indirect_direct[:, 0]
            c_mask = conf_scores.gt(conf_thresh)
            scores = conf_scores[c_mask]
            boxes = output_carplate_indirect_direct[:, 1:5]
            corners = output_carplate_indirect_direct[:, 5:]
            ...
            return output, output_indirect, output_direct
        else:
...
def build_ssd(phase, size=300, size_2=56, num_classes=21, expand_num=3):
    ...
    return SSD_indirect_direct(phase, size, size_2, base_, extras_, head_, base_2_, head_2_,
